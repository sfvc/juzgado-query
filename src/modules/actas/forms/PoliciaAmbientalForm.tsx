...
export const PoliciaAmbientalForm = ({ acta }: Props) => {
  ...
  const methods = useForm<IActaForm>({
    defaultValues: {
      ...
      tipo_acta: ACTAS.POLICIA_AMBIENTAL,
  ...
  return (
    <React.Fragment>
      <h1 className='font-semibold text-3xl mb-4 text-center dark:text-white'>Acta de Policia Ambiental</h1>

      <FormProvider {...methods}>
        <form className='flex flex-col gap-4 w-full' onSubmit={methods.handleSubmit(onSubmit)}>
          <ActaData tipoActa={ACTAS.POLICIA_AMBIENTAL}/>
